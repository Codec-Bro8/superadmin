...
const OnboardedChurchDetail: React.FC = () => {
  const { churchVideos, audios } = useApiStore();
  const router = useRouter();
  const { name } = router.query as RouterQuery;

  const churchDataVideos =
    churchVideos.filter((church) => church.churchName === name) || [];
  ...
  const handleCloseModal = () =>
    setModalState({ isOpen: false, message: "", onDelete: () => {} });
        ...
        <Tabs variant="soft-rounded">
          ...
          <TabPanels>
            <TabPanel className="flex gap-3">
              {churchDataVideos.map((item, index) => (
                <Box key={index} p={4} fontSize="small" w="20%">
                  <Box className="relative">
                    ...
                    <Text
                      as="span"
                      className="absolute bg-shade-1 top-1 right-3 rounded-md p-2"
                    >
                      20:00
                    </Text>
                  </Box>
                  <Box>
                    ...
                    <Box className="flex justify-between items-start">
                      <Text fontSize="x-small">{item.date}</Text>
                      <Button
                        bg="none"
                        width="fit-content"
                        color="white.1"
              ...
              {/* {churchData.blogs.map((blog: Blog, index: number) => (
                <Box key={index} p={4} fontSize="small" w="20%">
                  <Box className="relative rounded-xl">
                    ...
                    <Text
                      as="span"
                      className="absolute bg-shade-1 top-1 right-3 rounded-md p-2 text-xs"
                    >
                      {blog.date}
                    </Text>
                    <Box
            ...
            <TabPanel className="flex gap-3">
              {/* {churchData.devotionals.map(
                (devotional: Devotional, index: number) => (
                  <Box
                    key={index}
                    p="1rem"
                    bg="shade.3"
